...
export const StatsView: React.FC<StatsViewProps> = ({ evaluations, employees, onClose }) => {
  ...
  const avgScore = totalEvaluations > 0 ? (evaluations.reduce((acc, ev) => {
    const score = ev.criteria.reduce((sum, c) => sum + c.score, 0) / ev.criteria.length;
    return acc + score;
  }, 0) / totalEvaluations).toFixed(1) : '0.0';
  ...
  evaluations.forEach(ev => {
    ...
    if (emp) {
      ...
      const score = ev.criteria.reduce((sum, c) => sum + c.score, 0) / ev.criteria.length;
      deptPerformance[dept].total += score;
  ...
  evaluations.forEach(ev => {
    const s = ev.criteria.reduce((sum, c) => sum + c.score, 0) / ev.criteria.length;
    if (s >= 8) dist.high++;
