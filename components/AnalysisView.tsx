...
export const AnalysisView: React.FC<AnalysisViewProps> = ({ employee, criteria, analysis, onReset, evaluatorId }) => {
  ...
  const allRoles = [
    { jobTitle: employee.jobtitle || employee.jobTitle, department: employee.department, reportsTo: employee.reportsto || employee.reportsTo },
    ...(employee.additionalRoles || employee.additionalroles || [])
  ];

  const relevantRoles = evaluatorId
    ? allRoles.filter(r => (r.reportsto || r.reportsTo) === evaluatorId)
    : allRoles;
  ...
  const handleShare = async () => {
    const rolesStr = relevantRoles.map(r => `${r.jobTitle || r.jobtitle} (${r.department})`).join(', ');
    const textData = `
          ...
          <div className="md:col-span-2">
            ...
            <div className="font-bold text-slate-300 leading-tight text-sm sm:text-base space-y-1">
              {relevantRoles.map((r, i) => (
                <div key={i}>{r.jobtitle || r.jobTitle} <span className="text-[10px] opacity-60 italic">({r.department})</span></div>
              ))}
            </div>
          </div>
          <div className="sm:text-right">
            <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">Cumplimiento ISO</p>
            <span className={`inline-block px-3 sm:px-4 py-1 sm:py-1.5 rounded-xl text-[10px] sm:text-xs font-black uppercase tracking-widest ${analysis.isoComplianceLevel.includes('Excelente') || analysis.isoComplianceLevel.includes('Alto')
