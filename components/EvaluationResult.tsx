    ...
    <div className="max-w-4xl mx-auto p-6 animate-fade-in">
      ...
      <div className="no-print bg-slate-900 p-8 rounded-[2rem] border border-slate-800 flex justify-between items-center mb-8">
        <div>
          ...
          <h2 className="text-white font-black text-2xl uppercase tracking-tighter">{employee.name}</h2>
        </div>
        <button 
          onClick={() => window.print()} 
          className="bg-orange-600 text-white px-8 py-4 rounded-2xl font-black uppercase text-xs flex items-center gap-2 hover:bg-orange-500 transition-all"
        >
          <Printer size={18} /> Imprimir Reporte A4
        </button>
      </div>

      {/* Este es el componente que se vuelve blanco al imprimir */}
