...
import { Plus, Trash2, Building2, Settings, Save, X, Edit2, Loader2 } from 'lucide-react';
...
export const AdminPanel: React.FC<AdminPanelProps> = ({ employees, departments, criteria, evaluations, onClose, onSave, isSaving, onRefresh }) => {
  ...
  }, [employees, departments, criteria, evaluations]);
  ...
  const [newEmp, setNewEmp] = useState<Partial<Employee>>({ name: '', department: (localDepartments && localDepartments.length > 0) ? localDepartments[0] : '', jobTitle: '', reportsTo: '', additionalRoles: [] });
  const [newDeptName, setNewDeptName] = useState('');
  const [deletingId, setDeletingId] = useState<string | null>(null);
  const formRef = useRef<HTMLDivElement>(null);

  const resetForm = () => {
    setNewEmp({ name: '', department: (localDepartments && localDepartments.length > 0) ? localDepartments[0] : '', jobTitle: '', reportsTo: '', additionalRoles: [] });
    setEditingId(null);
  };

  const handleSaveEmployee = () => {
    const jt = newEmp.jobTitle;
    ...
    const normalizedEmp: Employee = {
      ...
      jobTitle: jt,
      reportsTo: newEmp.reportsTo || '',
      additionalRoles: newEmp.additionalRoles || [],
      averageScore: newEmp.averageScore || 0
    };
  ...
  const handleSaveDeptEdit = () => {
    ...
    const updatedEmps = localEmployees.map(emp => ({ ...emp, department: emp.department === editingDeptName ? editDeptValue.trim() : emp.department }));
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex flex-col">
          ...
          <>
            <div ref={formRef} className={`p-5 rounded-[2rem] border ${editingId ? 'bg-amber-500/5 border-amber-500/30' : 'bg-slate-900 border-slate-800 shadow-xl'}`}>
              <div className="space-y-4">
                ...
                <input placeholder="Puesto Principal" value={newEmp.jobTitle || ''} onChange={e => setNewEmp({ ...newEmp, jobTitle: e.target.value })} className="w-full bg-slate-800 p-3 rounded-2xl text-white text-sm border border-slate-700 outline-none focus:border-orange-500/50" />
              ...
              <div className="mt-8 space-y-4">
                ...
                {(newEmp.additionalRoles || []).map((role, idx) => (
                  <div key={idx} className="p-4 bg-slate-800/50 rounded-2xl border border-slate-700 relative space-y-4">
                    ...
                    <button onClick={() => setNewEmp({ ...newEmp, additionalRoles: (newEmp.additionalRoles || []).filter((_, i) => i !== idx) })} className="absolute -top-3 -right-2 bg-red-600 p-2 rounded-full text-white shadow-xl hover:bg-red-500 transition-all"><Trash2 size={14} /></button>
                  </div>
                ))}
              </div>
                ...
                <div key={emp.id} className="bg-slate-900/50 p-4 rounded-3xl border border-slate-800 flex justify-between items-center shadow-sm hover:border-slate-700 transition-all">
                  ...
                  <div className="flex gap-2 ml-4">
                    ...
                    <button 
                      ...
                      className="p-3 bg-red-950/20 text-red-500 rounded-2xl hover:bg-red-900/40 transition-all disabled:opacity-50"
                    >
                      {deletingId === emp.id ? <Loader2 className="animate-spin" size={18} /> : <Trash2 size={18} />}
                    </button>
                  </div>
                </div>
              ))}
          ...
          <div className="space-y-6 animate-fade-in">
            <div className="bg-slate-900 p-6 rounded-[2rem] border border-slate-800 shadow-xl">
              ...
              <div className="flex flex-col gap-3">
                ...
                <button onClick={() => { if (!newDeptName.trim()) return; const u = [...localDepartments, newDeptName.trim()]; onSave(localEmployees, u, localCriteria, localEvaluations); setNewDeptName(''); }} className="w-full bg-orange-600 text-white py-4 rounded-2xl font-black uppercase text-xs shadow-lg shadow-orange-900/20 flex items-center justify-center gap-2 hover:bg-orange-500 transition-all">
                  ...
                  <div className="flex gap-2 ml-4">
                    ...
                    <button 
                      onClick={() => {
                        if (deletingId) return;
                        setDeletingId(dept);
                        const promise = fetch(`/api/data?type=department&name=${encodeURIComponent(dept)}`, { method: 'DELETE' });
                        
                        toast.promise(promise, {
                          loading: 'Borrando área...',
                          success: (res) => {
                            if (!res.ok) throw new Error('Fallo al borrar');
                            onRefresh();
                            return 'Área borrada';
                      ...
                      disabled={deletingId === dept}
                      className="p-3 bg-red-950/20 text-red-500 rounded-2xl hover:bg-red-900/40 transition-all disabled:opacity-50"
                    >
                      {deletingId === dept ? <Loader2 className="animate-spin" size={18} /> : <Trash2 size={18} />}
                    </button>
                  </div>
                </div>
              ))}
                  ...
                  <div key={ev.id} className="bg-slate-900 p-4 rounded-3xl border border-slate-800 flex justify-between items-center shadow-sm">
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex items-center gap-2 text-[10px] text-slate-500 font-bold uppercase">
                        ...
                        <span className="text-orange-500">Score: {ev.criteria.reduce((acc, c) => acc + c.score, 0) / ev.criteria.length}</span>
                      </div>
                    </div>
                    <button
                      onClick={() => {
                        if (deletingId) return;
                        setDeletingId(ev.id);
                        const promise = fetch(`/api/evaluations/${ev.id}`, { method: 'DELETE' });
                      ...
                      className="p-3 bg-red-950/20 text-red-500 rounded-2xl hover:bg-red-900/40 transition-all border border-red-500/10 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      {deletingId === ev.id ? <Loader2 className="animate-spin" size={18} /> : <Trash2 size={18} />}
                    </button>
                  </div>
                );
              })
      ...
      <div className="p-4 border-t border-slate-800 bg-slate-900 sticky bottom-0 z-[110] shadow-[0_-10px_20px_rgba(0,0,0,0.5)]">
        <button 
          onClick={() => { onSave(localEmployees, localDepartments, localCriteria, localEvaluations); onClose(); }} 
          className="w-full bg-slate-100 text-slate-950 py-4 rounded-2xl font-black uppercase text-xs shadow-xl flex items-center justify-center gap-2 hover:bg-white active:scale-95 transition-all disabled:opacity-50"
          disabled={isSaving}
        >
          {isSaving ? <><Loader2 className="animate-spin" /> Guardando...</> : 'Guardar Todo y Finalizar'}
        </button>
      </div>
    </div>
  );
