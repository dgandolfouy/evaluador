            ...
            <div ref={formRef} className={`p-5 rounded-[2rem] border ${editingId ? 'bg-amber-500/5 border-amber-500/30' : 'bg-slate-900 border-slate-800 shadow-xl'}`}>
              ...
              <div className="mt-8 space-y-4">
                <div className="flex justify-between items-center border-b border-slate-800 pb-2">
                  <span className="text-[10px] font-black text-slate-500 uppercase">Funciones Extra (ISO 9001)</span>
              ...
              <button onClick={handleSaveEmployee} className="w-full bg-orange-600 text-white py-4 rounded-2xl mt-6 text-xs font-black uppercase shadow-lg">{editingId ? 'Confirmar' : 'Registrar'}</button>
            </div>

            <div className="space-y-3">
              {localEmployees.map(emp => (
                <div key={emp.id} className="bg-slate-900/50 p-4 rounded-3xl border border-slate-800 flex justify-between items-center">
                  <div className="min-w-0 flex-1"><div className="font-black text-white text-sm truncate uppercase tracking-tighter">{emp.name}</div><div className="text-[10px] text-slate-500 font-bold">{emp.jobTitle}</div>
                    {emp.additionalRoles && emp.additionalRoles.length > 0 && <div className="flex gap-1 mt-1 overflow-x-auto no-scrollbar">{emp.additionalRoles.map((r, i) => <span key={i} className="px-2 py-0.5 bg-orange-500/10 text-orange-500 text-[8px] font-black rounded-full border border-orange-500/20 whitespace-nowrap">+{r.jobTitle}</span>)}</div>}
                  </div>
