  ...
  return (
    <div className="max-w-4xl mx-auto p-4 sm:p-10 space-y-12 animate-fade-in pb-40">
      ...
      <div className="bg-slate-900 p-10 rounded-[3rem] border border-slate-800 shadow-2xl flex justify-between items-center">
        ...
        {canEditStructure && (
          <div className="bg-orange-600/20 px-4 py-2 rounded-full border border-orange-600/40 flex items-center gap-2">
            ...
            <span className="text-[10px] font-black text-orange-500 uppercase tracking-widest">Calidad</span>
          </div>
        )}
      </div>
      ...
      <div className="space-y-16">
        {criteria.map((c, idx) => (
          <div key={c.id} className="bg-slate-900 p-10 sm:p-14 rounded-[4rem] border border-slate-800 shadow-2xl relative">
            {/* Título: Blanco puro sobre fondo oscuro */}
            <div className="mb-10 border-b border-slate-800 pb-8">
              <h4 className="text-2xl font-black text-white uppercase tracking-widest mb-4">{c.name}</h4>
            ...
            <div className="space-y-14">
              ...
              <div className="bg-slate-950 p-10 rounded-[3rem] border border-white/5 shadow-inner">
                <div className="flex justify-between items-end mb-10">
                   <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em]">Puntaje de Desempeño</p>
              ...
              <div className="space-y-4">
                <span className="text-xs font-black text-slate-500 uppercase tracking-widest px-4">Evidencia ISO 9001</span>
                <textarea 
                  value={c.feedback || ''}
                  onChange={(e) => {
                    const newC = [...criteria];
      ...
      <div className="flex gap-6 pt-10">
        <button onClick={onCancel} className="flex-1 bg-slate-900 text-slate-500 py-8 rounded-[2.5rem] font-black uppercase text-xs border border-slate-800">Cancelar</button>
        <button onClick={() => onComplete(criteria)} className="flex-[2] bg-orange-600 text-white py-8 rounded-[2.5rem] font-black uppercase text-xs shadow-2xl flex items-center justify-center gap-4 hover:bg-orange-500 transition-all">
          <CheckCircle2 size={24} /> Finalizar Evaluación
        </button>
      </div>
    </div>
  );
