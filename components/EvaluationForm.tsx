...
export const EvaluationForm = ({ employee, onComplete, onCancel }: any) => {
  ...
  return (
    <div className="max-w-5xl mx-auto p-4 sm:p-10 space-y-12 pb-40 animate-fade-in">
      {/* CABEZAL LIMPIO */}
      <div className="bg-slate-900 p-10 rounded-[3rem] border border-slate-800 shadow-2xl">
        <button onClick={onCancel} className="text-slate-500 mb-2 flex items-center gap-1 text-[10px] font-black uppercase hover:text-white transition-colors"><ArrowLeft size={14}/> Volver</button>
        ...
        {criteria.map((c, idx) => (
          <div key={c.id} className="bg-slate-900 p-12 rounded-[4rem] border border-slate-800 shadow-2xl">
            ...
            <div className="space-y-12">
              {/* PUNTAJE: Sin cuadros amontonados */}
              ...
              {/* EVIDENCIA: Único cuadro donde podés escribir */}
              <div className="space-y-4">
                ...
                <textarea 
                  ...
                  placeholder="Escriba aquí los hechos técnicos o situaciones observadas..."
                  className="w-full h-44 bg-slate-950 border border-slate-800 rounded-[2.5rem] p-8 text-white text-lg font-medium outline-none focus:border-orange-600/50 transition-all resize-none shadow-inner"
                />
              </div>
            </div>
          </div>
      ...
      <button onClick={() => onComplete(criteria, { summary: "Evaluación manual" })} className="w-full bg-orange-600 text-white py-10 rounded-[3.5rem] font-black uppercase text-base shadow-2xl hover:bg-orange-500 transition-all">
