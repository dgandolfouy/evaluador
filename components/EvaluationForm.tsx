...
import { analyzeEvaluation } from '../services/geminiServices';
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-10 pb-40 text-white animate-fade-in">
      <div className="bg-slate-900 p-8 rounded-[2rem] border border-slate-800 shadow-2xl">
        ...
        <h2 className="text-orange-500 font-black text-4xl uppercase tracking-tighter">{employee.name}</h2>
        <p className="text-white/60 text-xs font-bold uppercase">{employee.jobTitle}</p>
      </div>
          ...
          <div key={c.id} className="bg-slate-900 p-10 rounded-[3rem] border border-slate-800 shadow-2xl flex flex-col gap-8">
            ...
            <div className="bg-slate-950 p-8 rounded-[2rem] border border-white/5 shadow-inner">
              <div className="flex justify-between items-end mb-8">
                 ...
                 <p className="text-6xl font-black text-white leading-none">{c.score}<span className="text-orange-600 text-xl ml-1">/10</span></p>
              </div>
            ...
            <textarea 
              ...
              placeholder="Evidencia técnica para el reporte..."
              className="w-full h-32 bg-slate-950 border border-slate-800 rounded-[2rem] p-6 text-white text-lg focus:border-orange-600 outline-none transition-all resize-none shadow-xl"
            />
          </div>
        ))}
      </div>
      ...
      <button onClick={handleFinish} disabled={loading} className="w-full bg-orange-600 py-10 rounded-[3.5rem] font-black uppercase text-xl shadow-2xl flex items-center justify-center gap-4 hover:bg-orange-500 active:scale-95 transition-all">
        ...
        {loading ? 'Generando Informe ISO...' : 'Finalizar y Guardar'}
      </button>
    </div>
  );
};
