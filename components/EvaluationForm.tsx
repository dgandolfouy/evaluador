...
export const EvaluationForm = ({ employee, initialCriteria, currentUser, onUpdateGlobalCriteria, onComplete, onCancel }: any) => {
  ...
  return (
    <div className="max-w-3xl mx-auto p-4 sm:p-6 space-y-8 pb-32">
      <div className="bg-slate-900 p-8 rounded-[2rem] border border-slate-800 flex justify-between items-start">
        <div>
        ...
        {canEditStructure && (
          <div className="bg-orange-600/10 px-3 py-1 rounded-full border border-orange-600/20 flex items-center gap-2">
            ...
            <span className="text-[10px] font-black text-orange-500 uppercase">Calidad</span>
          </div>
        )}
      </div>
      ...
      <div className="space-y-10">
        {criteria.map((c, idx) => (
          <div key={c.id} className="bg-slate-900 p-6 sm:p-10 rounded-[2.5rem] border border-slate-800 relative group">
            {canEditStructure && (
            ...
            <div className="flex flex-col gap-10">
              <div className="space-y-6 bg-slate-950/50 p-6 rounded-3xl border border-slate-800/50">
                <div className="flex justify-between items-center mb-2">
                   <span className="text-[10px] font-black text-slate-500 uppercase">Puntaje</span>
        ...
        {canEditStructure && !isAdding && (
          <button onClick={() => setIsAdding(true)} className="w-full py-8 border-2 border-dashed border-slate-800 rounded-[2.5rem] text-slate-500 font-black uppercase text-xs hover:text-orange-500 transition-all">+ Crear Criterio RR</button>
        )}
        {isAdding && (
          <div className="bg-slate-900 p-8 rounded-[2.5rem] border border-orange-600/30 space-y-4">
            <input placeholder="Nombre" value={newCrit.name} onChange={e => setNewCrit({...newCrit, name: e.target.value})} className="w-full bg-slate-950 p-4 rounded-2xl text-white text-sm" />
            <textarea placeholder="Descripción" value={newCrit.description} onChange={e => setNewCrit({...newCrit, description: e.target.value})} className="w-full bg-slate-950 p-4 rounded-2xl text-white text-sm h-24" />
            <div className="flex gap-2"><button onClick={() => setIsAdding(false)} className="flex-1 py-3 bg-slate-800 rounded-xl">Cancelar</button><button onClick={handleAddNew} className="flex-1 py-3 bg-orange-600 rounded-xl text-white">Guardar</button></div>
          </div>
        )}
      </div>

      <button onClick={() => onComplete(criteria)} className="w-full bg-orange-600 text-white py-6 rounded-3xl font-black uppercase text-xs shadow-2xl flex items-center justify-center gap-4">
