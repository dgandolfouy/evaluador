...
export const Dashboard = ({ evaluations, employees, currentUser, onNew, onQuickStart, onView }: any) => {
  ...
  const totalEval = mySubordinates.filter((e: Employee) => evaluations.some((ev: any) => (ev.employeeid || ev.employeeId) === e.id)).length;
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {mySubordinates.map((emp: Employee) => {
          const lastEval = evaluations.find((ev: any) => (ev.employeeid || ev.employeeId) === emp.id);
          return (
            <div key={emp.id} className={`p-6 rounded-[2.5rem] border ${lastEval ? 'bg-slate-900/40 border-green-500/20' : 'bg-slate-900 border-slate-800'} transition-all hover:scale-[1.02] flex flex-col`}>
              ...
              <p className="text-slate-500 text-[11px] font-bold uppercase tracking-widest mb-6 min-h-[1.5em]">{emp.jobtitle || emp.jobTitle}</p>

              <div className="mt-auto border-t border-slate-800/50 pt-5">
                {lastEval ? (
                  <div className="space-y-4">
                    <div className="flex items-center gap-2 text-slate-400 text-[10px] font-bold uppercase">
                      <Calendar size={14} className="text-orange-500/50" /> {new Date(lastEval.date).toLocaleDateString()}
                    </div>
                    <button onClick={() => onView(lastEval)} className="w-full py-3.5 bg-slate-800 hover:bg-slate-700 text-white rounded-2xl text-[11px] font-black uppercase transition-all shadow-lg shadow-black/20">Ver Reporte</button>
                  </div>
                ) : (
                  <button onClick={() => onQuickStart(emp.id)} className="w-full py-4 bg-orange-600/10 hover:bg-orange-600 text-orange-500 hover:text-white border border-orange-500/20 hover:border-transparent rounded-2xl text-[11px] font-black uppercase transition-all">Iniciar Ahora</button>
                )}
              </div>
            </div>
          );
